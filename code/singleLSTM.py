...
setnum = 1#1 is opp79, 2 is pamap2, 3 is skoda, 4 is dg
if setnum == 1:
...
drop_channel = 1# 0 don't use drop channel, 1 means use drop channel
...
test_len = len(test_x)
max_num_change = (dim*0.2)//1 + 1
print('max_num_change:', max_num_change)
			...
			while pos_end < train_len:
				...
				if drop_channel == 1:
					for j in range(batch_size):
						num_change = np.random.randint(0,max_num_change)
						dim_location_change = np.random.randint(0,dim-num_change)
						inputs[j,:,dim_location_change:dim_location_change+num_change] = 0#also can change to other values
				################################
